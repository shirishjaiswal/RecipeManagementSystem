...
public class UserService {
    ...
    public SignInResponse signIn(SignInRequest signInRequest) {
        ...
        if(isLoggedIn(user.getId())) {
            return new SignInResponse("Already Signed In", "");
        }
        String encryptedPassword = null;
        try {
    ...
    public ResponseEntity<String> signOut(String email) {
        User isUserValid = checkSecurity(email);
        if(isUserValid == null) {
    ...
    public User checkSecurity (String email) {
        //Is User Present in Database?
        User isUserPresent = userRepo.findFirstByEmail(email);
        if (isUserPresent == null) {
            return null;
        }
        else if(!isLoggedIn(isUserPresent.getId())) {
            return null;
        }
        else return isUserPresent;
    }
}
